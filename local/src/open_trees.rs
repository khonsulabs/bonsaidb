...
impl OpenTrees {
    ...
    pub fn open_trees_for_document_change(
        ...
        self.open_tree::<VersionedTreeRoot>(&document_tree_name(database, collection));

        if let Some(views) = schema.views_in_collection(collection) {
            for view in views {
                let view_name = view.view_name()?;
                if view.unique() {
                    self.open_tree::<UnversionedTreeRoot>(&view_omitted_docs_tree_name(
                        database, &view_name,
                    ));
                    self.open_tree::<UnversionedTreeRoot>(&view_document_map_tree_name(
                        database, &view_name,
                    ));
                    self.open_tree::<UnversionedTreeRoot>(&view_entries_tree_name(
                        database, &view_name,
                    ));
                } else {
